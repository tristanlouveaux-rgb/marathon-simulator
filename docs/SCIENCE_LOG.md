...
## §S — HYROX Race Simulation Sessions (2026-09-01)

**What it is.** Two session types that rehearse the HYROX race itself rather
than a slice of it: a full simulation (8 × 1 km run + all 8 stations in fixed
race order) and a half simulation (the same complete sequence at half volume).
Both are generated by `generateHyroxSimulation` in
`src/workouts/hyrox-generators.ts`.

### Why a simulation is a distinct session, not a longer brick

The existing brick session is *N* rounds of 1 km run into **one repeated
station**, chosen by phase. That trains compromised running, but it never
reproduces two properties of the race that the evidence identifies as decisive:

1. **Fixed station order.** HYROX is always the same eight stations in the same
   sequence. Fatigue at wall balls is a function of everything that preceded it.
2. **The late-race limiter.** In a full simulated HYROX event, wall balls
   produced the highest heart rate, blood lactate and RPE of any station
   (Brandt et al., 2025). A brick that rotates stations by phase will, on most
   weeks, never place wall balls under eight rounds of accumulated fatigue.

The simulation therefore preserves the whole ordered sequence. This is also why
the half simulation halves *volume* rather than dropping to four stations:
running the first four stations at full distance would cut the exact part of the
race that discriminates performance.

### Half-distance scaling

Half-simulation volumes are `STATION_HALF_VOLUME` in `hyrox-benchmarks.ts`,
which is exactly half of `STATION_RACE_VOLUME` and identical to the half-test
protocol the station benchmark calibration card already prescribes (SkiErg 500 m,
sled push 25 m, burpee broad jumps 40 m, wall balls 50 reps, and so on). No new
scaling constant was introduced: the app already had one, and reusing it means a
half-simulation split is directly interchangeable with a half-test split.

### Target time

Duration is not a user-chosen slider value. It is derived:

```
target = 8 × runLegSec + Σ(stationSec) + roxzoneSec
```

with `runLegSec` from the athlete's calibrated `hyroxRunPaceSecKm` (falling back
to `SEED_RUN_PACE_SEC_KM[band]`), `stationSec` from their calibrated
`stationBenchmarks` (falling back to `STATION_SEED_TIMES_SEC[band]`), and
`roxzoneSec` from `SEED_ROXZONE_SEC[band]`, halved for the half simulation. This
is the same component sum `predictHyroxRace` uses for the forecast, minus venue
course factors, which do not apply to a training session in the athlete's own
gym. The target is therefore consistent with the forecast the athlete already
sees, and it is a genuine time to beat rather than an arbitrary session length.

### MTL

Per-component MTL uses the existing `computeComponentMTL(durationMin, sRPE,
component, externalLoadKg, bodyWeightKg)`, with:

- `SIM_RUN_SRPE = 8` for run legs
- `SIM_STATION_SRPE = 9` for station components

Stations rate above runs because the simulated-event data show stations
producing higher lactate and higher RPE than run segments (Brandt et al., 2025).
Both simulations use the same per-component values, so the half simulation's MTL
falls out at roughly half the full simulation's from halved duration alone. No
separate "easier effort" multiplier was invented for the half version.

Session-level RPE is 9 (full) and 8 (half). These sit above the existing brick
(7) and station density (7) on this file's own established 1-point scale, since
a race-distance simulation is a maximal effort and a brick is not.

**Flagged as a judgement call:** the 9 / 8 session-level split is reasoned from
the internal scale, not measured. The half simulation still completes the whole
sequence, so it does reach terminal wall-ball fatigue, just at roughly half the
accumulated work. If dogfooding shows the half simulation feels like a 9, the
value should move. It is a single constant in `hyrox-generators.ts`.

### Splits as calibration input

Recorded splits are written by `applySplitsToConfig` in
`src/calculations/hyrox-simulation-splits.ts` into
`stationBenchmarksSingles` / `stationBenchmarksDoubles`,
`stationBenchmarkHistory` (sources `simulation` / `half_simulation`), and
`hyroxRunPaceSecKm` with `hyroxRunPaceSource = 'user'`.

Two deliberate differences from `applyParsedBenchmarks`, which handles
auto-imported activities:

- **Not PB-only.** `applyParsedBenchmarks` only improves a benchmark, because it
  fires automatically off any matched activity and a slow training session
  should not degrade a calibrated race benchmark. A simulation is a deliberate
  test the athlete chose to run and record, so its result replaces the previous
  value even when slower. Keeping a stale faster time would overstate the
  forecast.
- **Recorded under fatigue.** `full_test` in the existing source enum means a
  fresh full-distance station attempt. Simulation splits are not fresh: they are
  measured under accumulated race fatigue. That is arguably a *better* input to
  a race forecast, which sums station times as they would occur in a race, but
  it is a different measurement and is tagged as such so the station history
  shows what kind of test produced each number.

### Known limitations

- **Doubling a half split assumes linear scaling.** Halving reps or distance
  does not halve time exactly: the second half of a station is slower under
  local fatigue, so a doubled half-test underestimates the full-distance time.
  This limitation is inherited from the existing half-test protocol, not
  introduced here, and applies identically to both.
- **Station seed times are population midpoints,** gender-averaged, Open
  division. An uncalibrated athlete's first simulation target carries all the
  error of the band assignment.
- **RoxZone time is a whole-session constant** distributed nowhere in
  particular. In a real race it is eight discrete transitions of unequal length.
- **No course factors.** Unlike the forecast, the simulation target applies no
  venue floor-surface, temperature or altitude adjustment, since it is run in
  the athlete's own gym.

**References**

- Brandt, K. et al. (2025). Physiological demands of a simulated HYROX event.
- Haddad, M. et al. (2017). Session-RPE method for training load monitoring.
- Olcina, G. et al. (2019). Cycling to running transition and compromised
